...
SHADOW_OLLAMA = os.getenv('ATLAS_DEMO_SHADOW_OLLAMA', 'http://10.0.0.33:11434')
OLLIE_MODEL = os.getenv('ATLAS_DEMO_OLLIE_MODEL', 'flow-gemma:latest')
ARCHIE_MODEL = os.getenv('ATLAS_DEMO_ARCHIE_MODEL', OLLIE_MODEL)
TIMEOUT = float(os.getenv('ATLAS_DEMO_TIMEOUT_S', '120'))
NUM_CTX = int(os.getenv('ATLAS_DEMO_NUM_CTX', '16384'))
RESPONDING_NODE = os.getenv('ATLAS_DEMO_RESPONDING_NODE', 'sarge.titan.internal')
...
OIL_FALLBACK = "ATLAS does not currently have a direct live per-barrel oil quote feed in this demo path. Public market coverage still shows oil as headline-sensitive and geopolitics-driven, but this view cannot verify the exact live Brent or WTI print with confidence."
TIMEOUT_FALLBACK = "ATLAS could not complete the live reasoning path inside the demo timeout window, but the system is online. Please retry the query or narrow it to the specific market, vessel class, or date range needed."

CURRENT_EVENT_TOKENS = (
    'last week', 'this week', 'latest', 'today', 'yesterday', 'headlines',
    'news', 'what happened', 'market update', 'update', 'at present',
    'present', 'currently', 'current'
)
...
OIL_PRICE_TOKENS = ('price', 'barrel', 'per barrel', '/ barrel', 'wti', 'brent')
GREETING_TOKENS = (
    'hi', 'hello', 'hey', 'hi atlas', 'hello atlas', 'hey atlas',
    'hi dr atlas', 'hello dr atlas', 'atlas hello', 'atlas hi'
)
PRESENCE_TOKENS = (
    'are you here', 'are you there', 'you there', 'are you online',
    'atlas are you here', 'atlas are you there', 'atlas online', 'you online'
)

def has_any(q:str, phrases:tuple[str, ...] | list[str]) -> bool:
    return any(phrase in q for phrase in phrases)

def is_greeting_or_presence_query(query:str) -> bool:
    q = query.lower().strip()
    q_simple = re.sub(r'[^a-z0-9\s]', '', q)
    return (
        q_simple in GREETING_TOKENS
        or has_any(q, PRESENCE_TOKENS)
        or any(phrase in q for phrase in ['hi son', 'hello son', "what's cooking", 'whats cooking', 'quick update', 'give me an update', 'status update'])
    )
...
def needs_live_news(query:str)->bool:
    ...
    ) or (has_any(q, OIL_MACRO_TOKENS) and has_any(q, OIL_PRICE_TOKENS))

def needs_live_oil_price(query:str)->bool:
    q=query.lower().strip()
    return has_any(q, OIL_MACRO_TOKENS) and has_any(q, OIL_PRICE_TOKENS)
...
def graceful_fallback_response(query:str, exc:Exception|None=None)->str:
    q = query.lower().strip()
    if is_greeting_or_presence_query(q):
        return "I'm here, Dr. Sharma. ATLAS is online and ready."
    if needs_live_oil_price(q):
        return OIL_FALLBACK
    if needs_live_bdi(q):
        return BDI_FALLBACK
    if needs_weekly_news(q):
        return "ATLAS is not yet using a live historical news retrieval path in this view, but the executive frame for the requested period remains centered on macro-sensitive shipping conditions, commodity-linked demand, congestion, and geopolitics."
...
def deterministic_response(query:str)->str|None:
    ...
    if any(phrase in q for phrase in ["are you able to research the internet", "can you browse", "can you look things up online", "can you research the internet", "can you browse the internet", "can you research online"]):
        return (
            "ATLAS can use live public web retrieval through Archie for current maritime, dry-bulk, Baltic, and macro-sensitive prompts in this view. Historical and proprietary coverage is still narrower, but current public-source research is active through the backend."
        )
...
async def gen(prompt:str, system:str, model:str, num_predict:int=1024)->str:
    payload={'model':model,'system':system,'prompt':prompt,'stream':False,'options':{'temperature':0.2,'num_predict':num_predict,'num_ctx':NUM_CTX}}
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        r=await client.post(f'{SHADOW_OLLAMA}/api/generate', json=payload)
        r.raise_for_status()
        response = (r.json().get('response') or '').strip()
        response = response.replace('<gMASK>', '').replace('</gMASK>', '').replace('<|file_separator|>', '').strip()
        return response
...
async def run_chain(qid:str):
    ...
    try:
        ...
        event(state,'FLOW_ROUTING','FLOW',f'routing_to_{RESPONDING_NODE}')
        message(state,'system','FLOW',f'Routing to {RESPONDING_NODE} fast path for live execution.', 'flow')
        await asyncio.sleep(0.12)
        event(state,'OLLIE_EXECUTING','OLLIE',f'{RESPONDING_NODE}_inference')
        direct = deterministic_response(q)
        deterministic_hit = direct is not None
        presence_hit = has_any(q_lower, PRESENCE_TOKENS)
        if deterministic_hit:
            ...
            if needs_live_bdi(q):
                try:
                    ...
                    bdi = None
                weekly_baltic = None
                if is_current_events_query(q) or 'baltic exchange' in q_lower:
                    try:
                ...
                if bdi and bdi.get('points') and bdi.get('date'):
                    ...
                    wants_weekly_baltic = any(token in q_lower for token in ('last week', 'this week', 'what happened', 'weekly', 'market update'))
                    if weekly_baltic and weekly_baltic.get('snippet') and wants_weekly_baltic:
                        prior_text = f" versus approximately {prior_points} on the prior comparison print" if prior_points else ""
                        ollie = (
                            f"Last week the Baltic Exchange tone weakened, with the Baltic Dry Index closing {bdi['date']} at {bdi['points']} points{prior_text}. "
                            f"Public weekly market reporting indicated bunker-cost pressure, corridor tension, and vessel-class dispersion mattered more than a clean demand breakout. "
                            "Executive read: the move still reflects bunker-cost pressure, geopolitical risk, and freight segmentation more than broad-based dry-bulk demand acceleration."
                        )
                    else:
                        ollie = (
            ...
            elif needs_live_oil_price(q):
                try:
                    market = await fetch_duckduckgo_result('current Brent crude oil price per barrel')
                except Exception:
                    market = None
                if market and market.get('snippet'):
                    ollie = (
                        f"ATLAS located a live public oil-market update: {market['snippet']} "
                        "Executive read: oil remains geopolitics-driven and shipping-relevant through bunker costs, risk premia, and voyage economics."
                    )
                else:
                    ollie = OIL_FALLBACK
            else:
        ...
        fallback = direct if deterministic_hit else deterministic_response(q)
        if ollie.strip() == INSUFFICIENT and is_greeting_or_presence_query(q):
            ollie = fallback or "I'm here, Dr. Sharma. ATLAS is online and ready."
        elif ollie.strip() == INSUFFICIENT and fallback is not None:
            ollie = fallback
        ...
        if deterministic_hit:
            ...
            state['responding_node']=RESPONDING_NODE
            state['coordinator_node']='sarge.titan.internal'
            state['duration_ms']=int((time.perf_counter()-start)*1000)
            save(state)
            with LOG_PATH.open('a') as fh:
                fh.write(json.dumps({'timestamp':now_iso(),'query_id':qid,'query':q,'agents':['CHARLIE','FLOW','OLLIE'] if presence_hit else ['CHARLIE','FLOW','OLLIE','ARCHIE'],'duration_ms':state['duration_ms'],'result_status':'complete','responding_node':RESPONDING_NODE,'coordinator_node':'sarge.titan.internal','query_hash':hashlib.sha256(q.encode()).hexdigest()[:16]}, ensure_ascii=False)+'\n')
            return
        ...
        save(state)
        with LOG_PATH.open('a') as fh:
            fh.write(json.dumps({'timestamp':now_iso(),'query_id':qid,'query':q,'agents':['CHARLIE','FLOW','OLLIE','ARCHIE'],'duration_ms':state['duration_ms'],'result_status':'complete','responding_node':RESPONDING_NODE,'coordinator_node':'sarge.titan.internal','query_hash':hashlib.sha256(q.encode()).hexdigest()[:16]}, ensure_ascii=False)+'\n')
    except Exception as exc:
        state=load(qid)
        fallback = graceful_fallback_response(q, exc)
        state['status']='complete'; state['response']=fallback; state['error']=str(exc); state['responding_node']=RESPONDING_NODE; state['duration_ms']=int((time.perf_counter()-start)*1000)
        message(state,'assistant','ARCHIE',fallback,'archie')
        event(state,'FINAL_RESPONSE','ARCHIE','fallback_after_error')
        save(state)
        with LOG_PATH.open('a') as fh:
            fh.write(json.dumps({'timestamp':now_iso(),'query_id':qid,'query':q,'agents':['CHARLIE','FLOW','OLLIE','ARCHIE'],'duration_ms':state['duration_ms'],'result_status':'fallback','error':str(exc),'responding_node':RESPONDING_NODE,'coordinator_node':'sarge.titan.internal'}, ensure_ascii=False)+'\n')

@app.get('/api/health')
async def health():
    return {'status':'ok','service':'atlas-demo-bridge','responding_node':RESPONDING_NODE,'coordinator_node':'sarge.titan.internal'}
...
async def query(body: DemoQuery, bg: BackgroundTasks):
    ...
    state={'query_id':qid,'status':'running','source':body.source,'principal':body.principal,'query':q,'events':[],'messages':[{'role':'user','label':body.principal or 'DR. SHARMA','text':q,'accent':'user','timestamp':now_iso()}],'created_at':now_iso(),'response':None,'responding_node':RESPONDING_NODE,'coordinator_node':'sarge.titan.internal','duration_ms':None}
    save(state)
    bg.add_task(run_chain,qid)
    return {'ok':True,'query_id':qid,'status':'running'}

def recent_runs(limit:int=12)->list[dict]:
    files = sorted(DATA_DIR.glob('atlas-demo-*.json'), key=lambda item: item.stat().st_mtime, reverse=True)
    records = []
    for file_path in files[:max(1, min(limit, 50))]:
        try:
            records.append(json.loads(file_path.read_text()))
        except Exception:
            continue
    return records

@app.get('/api/demo/recent')
async def recent(limit:int=12):
    return {'threads': recent_runs(limit)}
